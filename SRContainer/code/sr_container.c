...
int main(int argc, char **argv)
{
    ...
    //
    int cgroup_index = 1;
    struct cgroups_control new_cg[6]; //Declare array of cgroup, This allocates memory for new cgroups_control
    int new_cg_index = 0; //Index to track elements in new_cg

    //Declare and initialize cgroup_settings for blkio read/write, this allocates memory for blkio_settings
    struct cgroup_setting blkio_setting[2] = {(struct cgroup_setting) {
        .name = "blkio.throttle.read_bps_device", //read is element 0
        }, (struct cgroup_setting) {
        .name = "blkio.throttle.write_bps_device", //write is element 1
        }
    };
    int blkio_index = 1; //Index to track elements in blkio_setting
    ...
    while ((option = getopt(argc, argv, "c:m:u:C:s:p:M:r:w:H:")))
    {
        ...
        case 'C':;
            ...
            new_cg[new_cg_index] = cpu_cgroup;
            cgroups[cgroup_index++] = &new_cg[new_cg_index++];
            cgroups[cgroup_index] = NULL;
            break;
        case 's':;
            ...
            strcpy(cpuset_cgroup.settings[0]->value, "0-1");
            strcpy(cpuset_cgroup.settings[1]->value, optarg);
            new_cg[new_cg_index] = cpuset_cgroup;
            cgroups[cgroup_index++] = &new_cg[new_cg_index++];
            cgroups[cgroup_index] = NULL;
            break;
        case 'p':;
            ...
            new_cg[new_cg_index] = pids_cgroup;
            cgroups[cgroup_index++] = &new_cg[new_cg_index++];
            cgroups[cgroup_index] = NULL;
            break;
        case 'M':;
            struct cgroups_control memory_cgroup = {
            ...
            new_cg[new_cg_index] = memory_cgroup;
            cgroups[cgroup_index++] = &new_cg[new_cg_index++];
            cgroups[cgroup_index] = NULL;
            break;
        case 'r':;
            strcpy(blkio_setting[0].value, optarg);
            cgroups[0]->settings[blkio_index] = &blkio_setting[0];
            cgroups[0]->settings[++blkio_index] = NULL;
            break;
        case 'w':;
            strcpy(blkio_setting[1].value, optarg);
            cgroups[0]->settings[blkio_index] = &blkio_setting[1];
            cgroups[0]->settings[++blkio_index] = NULL;
            break;
        case 'H':
...
int countNum(struct cgroup_setting **set){
    int counter = 0;
    while(set[counter]!=NULL){
      counter++;
    }
    return counter;
}
