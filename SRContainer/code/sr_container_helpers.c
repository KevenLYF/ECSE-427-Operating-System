...
int setup_syscall_filters()
{
    ...
    // filter for unshare when flag CLONE_NEWUSER is used
    filter_set_status = seccomp_rule_add(
        seccomp_ctx,
        SCMP_FAIL,
        SCMP_SYS(unshare),
    ...
    // filter for clone when flag CLONE_NEWUSER is used
    filter_set_status = seccomp_rule_add(
        seccomp_ctx,
        SCMP_FAIL,
        SCMP_SYS(clone),
    ...
    // filter for chomod when flags ​S_ISUID are used 
    filter_set_status = seccomp_rule_add(
        seccomp_ctx,
        SCMP_FAIL,
        SCMP_SYS(chmod),
    ...
    if (filter_set_status) {
        ...
        fprintf(stderr, "seccomp could not add KILL rule for 'chmod' with flag S_ISUID: %m\n");
        return EXIT_FAILURE;
    }

    // filter for chomod when flags ​S_ISGID are used 
    filter_set_status = seccomp_rule_add(
        seccomp_ctx,
        SCMP_FAIL,
        SCMP_SYS(chmod),
    ...
    if (filter_set_status) {
        ...
        fprintf(stderr, "seccomp could not add KILL rule for 'chmod' with flag S_ISGID: %m\n");
        return EXIT_FAILURE;
    }
